...
func makePizza(pizzaNumber int) *Order {
	pizzaNumber++
...
func pizzeria(pizzaMaker *Producer) {
	...
	// this loop will continue to execute, truing to make pizzas,
	...
	for {
		...
		if currentPizza != nil {
			...
			// select is like a switch statement (just for channels)
			select {
			// we tried to make a pizza (we send something to the data channel -- a chan Order)
...
func main() {
	...
	// define a source for the rand generator
	source := rand.NewSource(time.Now().UnixNano())

	// use the source to generate random numbers
	rand.New(source)

	// print out a message
	color.Cyan("Ready to serve pizzas!")
	color.Cyan("----------------------")
