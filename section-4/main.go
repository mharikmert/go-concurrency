...
var eatTime = 3 * time.Second   // how long it takes to eatTime
var thinkTime = 3 * time.Second // how long a philosopher thinks

var eatOrder = []Philosopher{}
var eatOrderMutex sync.Mutex

func main() {
	// print out a welcome message
	fmt.Println("\n---------------------------")

	// remove the redundant newline character
...
func diningProblem(philosopher Philosopher, wg *sync.WaitGroup, forks map[int]*sync.Mutex, seated *sync.WaitGroup) {
	...
	eatOrderMutex.Lock()
	eatOrder = append(eatOrder, philosopher)
	eatOrderMutex.Unlock()
}
